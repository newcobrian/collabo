#
# use: generate all guide URLs into a file
#   % python etc/all-guide-urls.py > all-urls
# split into max 50 lines since FB's tool only takes 50 at a time
#   % split -l 50 all-urls
# you get files named xaa, xab, xab, xad.... put these into your clipboard one at a time
#   % cat xaa | pbcopy
# paste into the box in FB's batch invalidate tool at https://developers.facebook.com/tools/debug/sharing/batch/
...
for k in ref.iterkeys():
    print 'https://myviews.io/guide/%(k)s' % locals()
